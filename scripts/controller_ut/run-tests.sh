...
do
  ...
  # NOTE: error lines will be skipped
  file=$(echo $line | jq -r ".log_path, .xml_path " 2>/dev/null | grep -v "null")
  if [[ "$(echo $file |  wc -w )" != '0'  ]]; then
    for substring in $(echo $file  | wc -w) ; do
        dest=$( echo $(file)[$substring] | sed  "s~/root/contrail~$logs_path~g" )
        ech
    done
  fi
done < "$input"
