...
for utest in $(cat "$targets_file") ; do
  ...
  if ! timeout $TARGET_TIMEOUT "$scriptdir/run-tests.py" --less-strict -j $JOBS --skip-tests $DEV_ENV_ROOT/ci_skip_tests $utest &> $logs_path/$logfilename ; then
    res=1
...
scons -Q --warn=no-all --describe-tests --skip-tests $DEV_ENV_ROOT/ci_skip_tests $(cat $targets_file | tr '\n' ' ') > $test_list
