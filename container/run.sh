...
function package() {
    ...
    # We'll have to build all containers until frozen images are available for using
    make_containers="containers src-containers test-containers"
...
if [[ -z "$stage" ]] ; then
    ...
    # run selected stage unless we're in fast build mode and the stage is finished
    if ! finished_stage "$stage" || [[ $BUILD_MODE == "full" ]] || [[ $stage == "fetch" ]] || [[ $stage == "configure" ]]; then
