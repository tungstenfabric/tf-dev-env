...
echo "LOOK3: CONTAINER_REGISTRY $CONTAINER_REGISTRY"

load_tf_devenv_profile

echo "LOOK4: CONTAINER_REGISTRY $CONTAINER_REGISTRY"


source_env

echo "LOOK5: CONTAINER_REGISTRY $CONTAINER_REGISTRY"
...
function package() {
    ...
    echo "LOOK6: CONTAINER_REGISTRY $CONTAINER_REGISTRY"
    ...
    for container in ${unchanged_containers[@]}; do
        ...
        if [[ $(sudo docker pull "$FROZEN_REGISTRY/$container:$FROZEN_TAG") ]] ; then
            sudo docker tag "$FROZEN_REGISTRY/$container:$FROZEN_TAG" "$CONTAINER_REGISTRY/$container:$CONTRAIL_CONTAINER_TAG"
