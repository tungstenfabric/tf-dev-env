...
function package() {
    ...
    if [[ $BUILD_MODE == "fast" ]] && patches_exist ; then
        ...
        if [[ ! -z $changed_containers_projects ]] ; then
            ...
            # The following line shouldn't be make_containers="src-containers"
            # but until whole set of frozen Contrail containers is available in nexus
            # it has to build them each time
            make_containers="containers src-containers"
        fi
